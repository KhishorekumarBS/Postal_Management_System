...
app.post('/fetch_by_dept', function(req, res) {
        console.log("Here");
        srpschema.find({ 'dealinghand': req.body.key }).sort({date: -1}).exec(function(err, docs) { 
          console.log(docs);
          res.json(docs);
        });
    });
...
app.post('/fetch_by_id_year', function(req, res) {
        console.log("Here");
        srpschema.find({ '_id': req.body.key,'year': req.body.year }).exec(function(err, docs) { 
          console.log("-------------");
          console.log(docs); 
          res.json(docs);
        });
...
app.post('/fetch_by_year', function(req, res) {
        console.log("Here");
        srpschema.find({ 'year': req.body.year }).sort({date: -1}).exec(function(err, docs) { 
          console.log("-------------");
          console.log(docs); 
          res.json(docs);
        });
...
app.post('/update_with_id', function(req, res) {
        console.log("Here");
         ...
         if (err) {
            ...
            doc.dealinghand=req.body.dh;
            doc.courierno=req.body.cono;
            doc.backup=req.body.back;
            doc.actiondate=req.body.acdate;
            doc.save((err, doc) => {
            if (err) {
                console.log(err);
            }
...
app.post('/savee', function(req, res){
  ...
  new srpschema({
    ...
    ddnum: req.body.ddno,
    amount: req.body.amt,
    dealinghand: req.body.dh,
    courierno: req.body.cono,
    stockfile: '0',
    sendflag : "0",
    backup : "0",
    year : yearstr,
