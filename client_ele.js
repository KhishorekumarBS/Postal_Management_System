...
app.get('/', function(req, res) {
  res.render('displaycli3.ejs',{ message: '' });
});


app.post('/fetch_by_dept', function(req, res) {
        console.log("Here");
        srpschema.find({ 'department': dept_dh , 'sendflag' : '0'}).exec(function(err, docs) { 
          res.json(docs);
        });
    });
...
app.post('/verify', function(req, res) {
        ...
        var yearstr=dateobj.getFullYear();
         ...
         if (err) {
            ...
            doc.sendflag="1";
            doc.actiondate=daystr+"/"+monthstr+"/"+yearstr;
            doc.save((err, doc) => {
            if (err) {
                console.log(err);
            }
